...
# ╔═╡ 843bb526-e9a0-4f6f-bfd5-f59dd30efa47
using Makie.LaTeXStrings

# ╔═╡ f31b7e00-e1fd-4a2e-9786-b7f2534f6247
using Makie

# ╔═╡ e6bef3f9-1e80-442d-bf7c-849a050e436b
...
# ╔═╡ a6e1081f-03cc-4521-8679-4ee309bfdd10
using GraphMakie, NetworkLayout

# ╔═╡ ea9117aa-9e4f-4917-910d-1b34742f78f5
using Makie: automatic

# ╔═╡ 162b4644-530d-4f71-83c7-9fd7705f4e71
# ╠═╡ skip_as_script = true
#=╠═╡
using PlutoUI: TableOfContents
  ╠═╡ =#

# ╔═╡ 3cb2dcbb-699e-41a1-a09b-83e2af30a771
md"""
# Helpers for Financial Networks 
"""

# ╔═╡ 5108bb46-37af-41b3-bc60-d5b8c7171058
md"""
## Examples
"""

# ╔═╡ ba4e09f0-b21c-4f07-8ccf-e0994bac49a9
# ╠═╡ skip_as_script = true
#=╠═╡
import CairoMakie
  ╠═╡ =#

# ╔═╡ 2ebac279-e7cb-4237-81e4-df2721ff8ca1
bridge = 20

# ╔═╡ 49898eb9-5cd2-499d-a363-b06e106eff72
md"""
## Regular interbank market
"""

# ╔═╡ 84ae66cc-0fdb-40bc-871f-b4bf22eb102d
function is_regular(Y)
	vec(sum(Y, dims=1)) ≈ vec(sum(Y, dims=2))
end

# ╔═╡ 9df3db53-1126-4b75-ac91-e78273d5bd98
...
# ╔═╡ a708e0c9-69b3-4ba2-ac25-78e770840c16
...
# ╔═╡ c38db64e-5e7e-4a32-9df8-0c2cf9553fe5
begin
...
# ╔═╡ 5106df50-003e-44e3-9388-14c065d38e29
...
# ╔═╡ d4243b8b-f9e0-4888-a6e8-71109f4e595e
...
# ╔═╡ d042fc26-b900-45d8-af22-5fdd9fb7f585
function island_network(n_islands, n_banks_per_island, ȳ, γ)
	blocks = (γNetwork(n_banks_per_island, ȳ, γ).Y for _ in 1:n_islands)
	
	cat(blocks...,dims=(1,2))
end

# ╔═╡ 6944c528-c2b2-433f-a8d9-70f24cda6ff0
function island_network(n_banks::AbstractVector, ȳ, γ)
	blocks = (γNetwork(n, ȳ, γ).Y for n ∈ n_banks)
	
	cat(blocks...,dims=(1,2))
end

# ╔═╡ 296650ff-e8e1-43f3-97e0-c9af911c28e8
let
	...
	function IslandNetwork(n_islands, n_banks_per_island, ȳ; γ=0.0)
		...
		IslandNetwork(Y, ȳ, γ, n_islands, n_banks_per_island)
	end
	function IslandNetwork(n_banks::AbstractVector, ȳ; γ=0.0)
		Y = island_network(n_banks, ȳ, γ)	
		IslandNetwork(Y, ȳ, γ, length(n_banks), n_banks)
	end
end

# ╔═╡ f73e650a-05c1-414e-bb51-ec2c68243d58
...
# ╔═╡ 390ddb2b-e19f-4777-8788-0ed04600ad87
md"""
## Extending GraphMakie
"""

# ╔═╡ 44ad5a66-d9e8-4485-813a-1b5de824a1cf
begin
...
# ╔═╡ e118dc8d-c66b-4acb-a48c-047ec5ac718c
...
# ╔═╡ 6b395deb-a1d8-4b57-9b0b-d806baef47c5
...
# ╔═╡ 6d786aff-c67c-49bd-853b-a198e435f38e
function circular_graphplot(g; axis=(;), figure=(;), kwargs...)
	fig = Figure(; figure...)
...
# ╔═╡ 8fba314c-08ec-4e95-a96e-d611b5b2a6e5
md"""
## Numbered `graphplot`
"""

# ╔═╡ 64e7fdee-1e08-4425-b8ba-f6048ad9e465
figure(width, height) = (figure_padding = 0, font = "CMU", resolution = (width, height))

# ╔═╡ c5dc5548-e1c8-45a5-9c6b-5f732b38ffc8
figure(size=200) = figure(size, size)

# ╔═╡ 60a4cf53-c10f-4fc5-ab07-505abf94f6ed
begin
	n_banks = 3
	ȳ = 2.1
	...
	circular_graphplot(IM; figure=figure(200))
end

# ╔═╡ 19d47083-73e6-42dd-b399-b3521898eceb
...
# ╔═╡ 9c528522-c2d0-4af4-8031-b9bede46a3a6
function extended_box(points, margin = 0.05)
	...
	(extended_extrema(first.(points), x_margin)..., 
	 extended_extrema(last.(points), y_margin)...)
end

# ╔═╡ 4fb4016c-292f-4880-b524-cad2c5eda85d
...
# ╔═╡ 0d8d5312-d111-40a7-b88c-ec6bddacb895
function numbered_graphplot!(ax, g; 
	...
	node_marker = :circle, 
	node_font = "CMU", #automatic, #Makie.defaultfont(), 
	node_textsize = 16,
	arrow_attr = (;),
	kwargs...)
	n = nv(g)

	node_fontsize = node_textsize
	
    # Extract text sizes and layout accordingly
    label_sizes = [widths(text_bbox(string(label), node_fontsize, node_font, (:center, :center), 0f0, 0, 0)) for label in nlabels]
	...
	node_attr = (; 
        ...
        strokewidth = 1, markersize = node_markersize == automatic ? map(x -> x .+ node_fontsize, label_sizes) : node_markersize, 
        markerspace = :pixel,
    )
...
function numbered_graphplot(g; figure = (;), axis = (;), kwargs...)
	fig = Figure(; figure...)
	ax = Axis(fig[1,1]; axis...)

	if haskey(figure, :font)
		kwargs = (; kwargs..., node_font = figure.font)
	end
	
	numbered_graphplot!(ax, g; kwargs...)
...
# ╔═╡ a4435361-ed55-4e64-900c-36c853cb9c10
let
	n_islands = 2
	n_banks = 4
	components = [1:n_banks, (n_banks+1):(2n_banks)]
	ȳ = 2.1
	γ = 0.5
	Yᵢ = γNetwork(n_banks, ȳ, 0.99).Y

	blocks = (Yᵢ, Yᵢ)
	Y = cat(blocks...,dims=(1,2))

	scale_bridge = bridge
	# add links between blocks
	Y[1, n_banks+1]	= scale_bridge * ȳ / (n_banks - 1)
	Y[2n_banks-1,n_banks-1]	= scale_bridge * ȳ / (n_banks - 1)
	...
	for i ∈ 1:max_iter
		...
		if i == max_iter
			@warn "Did not converge after $i iterations"
		end
	...
	g = SimpleWeightedDiGraph(Y)
	numbered_graphplot(g, 
		extend_limits = 0.15,
		layout = g -> componentwise_circle(components, scale = 4),
...
# ╔═╡ 2fb8bc9f-5377-4d13-8c6f-73f114a59a22
md"""
# Appendix
"""

# ╔═╡ ed06d5a0-98a6-4112-9344-03e6ab7138e9
#=╠═╡
TableOfContents()
  ╠═╡ =#
